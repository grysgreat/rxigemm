...
extern __global__ void GEMMI8TCU(const int8_t* A, const int8_t* B, int8_t* C, int M, int N, int K);
            ...
            bool transA, bool transB, bool transC) 
{
  ...
  if(transA==GEMM_OP_T && transB==GEMM_OP_N && transC==GEMM_OP_T)
    GEMMI8TCU<BLOCK_M, BLOCK_N, BLOCK_K, WARP_M, WARP_N, 2, GEMM_OP_T, GEMM_OP_N, GEMM_OP_T><<<grid, block, 0, stream>>>(A, B, C, M, N, K);
